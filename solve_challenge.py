...
def _load_json_text(source: str) -> str:
    """Return the text for *source*, supporting @file and stdin shortcuts."""
...
def load_json_from_source(source: str, description: str) -> Any:
    """Decode JSON from *source*, raising ValueError with context."""

    try:
        text = _load_json_text(source)
    ...
    try:
        ...
        raise ValueError(f"Invalid JSON for {description}: {exc}") from exc
...
def parse_cli(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--items",
        help=(
            "JSON array of items to order instead of performing the GET request. "
            "Prefix with @ to read from a file or use '-' to read from stdin."
        ),
    )
    parser.add_argument(
        "--challenge",
        help=(
            "Full challenge JSON payload (with items/token). Prefix with @ to read "
            "from a file or use '-' to read from stdin."
        ),
    )
    parser.add_argument(
        "--no-submit",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if (args.proxy_user or args.proxy_password) and not args.proxy:
        ...
        return 64
    if args.challenge and (args.items or args.token):
        print(
            "--challenge cannot be combined with --items/--token", file=sys.stderr
        )
        return 64
    ...
    if args.challenge:
        try:
            payload = load_json_from_source(args.challenge, "--challenge")
        except ValueError as exc:
            print(str(exc), file=sys.stderr)
            return 64
        if not isinstance(payload, dict):
            print("--challenge must decode to a JSON object", file=sys.stderr)
            return 64
        items = payload.get("items")
        token = payload.get("token", "")
        if not isinstance(items, list) or not all(isinstance(it, str) for it in items):
            print(
                "--challenge payload must include an 'items' array of strings",
                file=sys.stderr,
            )
            return 64
        if not isinstance(token, str):
            print("--challenge payload must include a string token", file=sys.stderr)
            return 64
    elif args.items:
        try:
            items = load_json_from_source(args.items, "--items")
        except ValueError as exc:
            print(str(exc), file=sys.stderr)
            return 64
        if not isinstance(items, list) or not all(isinstance(it, str) for it in items):
            print("--items must decode to a JSON array of strings", file=sys.stderr)
            return 64
        token = args.token or ""
    else:
        try:
